...
import PortfolioInsights from './ProfolioInsights';
...
export const Dashboard = () => {
    ...
    const filteredStocks = useMemo(() => {
        let stocks = data?.portfolio?.stocks;
        if (!stocks) return [];
        return stocks.filter((item) => {
            const matchesSearch = item?.tradingsymbol?.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center min-h-screen bg-slate-50">
                ...
                <p className="text-slate-500 mt-4 font-medium tracking-wide">Analyzing your portfolio...</p>
            </div>
        );
    }
                ...
                <div className="max-w-[1400px] mx-auto px-6 py-4 flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-4">
                        <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-orange-50 rounded-lg border border-orange-100">
                            ...
                            <span className="text-xs font-semibold text-orange-700">Vicky G.</span>
                        </div>
                        <button onClick={handleLogout} className="p-2 rounded-lg hover:bg-red-50 text-slate-400 hover:text-red-500 transition">
                            <LogOut size={18} />
                        </button>
                    </div>
                </div>
            ...
            <main className="max-w-[1400px] mx-auto px-4 md:px-6 py-8">
                {activeTab === 'portfolio' ? (
                    <div className="space-y-8 animate-in fade-in duration-500">
                        ...
                        {data?.portfolio?.summary && (
                            <StatsHero 
                                ...
                                otherdata={{ 
                                    diversification_score: data.portfolio.diversification_score, 
                                    risk_profile: data.portfolio.risk_profile
                                }} 
                            />
                        )}
                        {data?.portfolio?.protfolio_insights && 
                            <PortfolioInsights insights={data?.portfolio?.protfolio_insights} />
                        }   
                        ...
                        <div className="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm">
                            <StockFilters
                                ...
                                uniqueSectors={[...new Set(data?.portfolio?.stocks?.map(s => s.sector))].filter(Boolean)}
                            />
                        </div>
                        ...
                        {filteredStocks.length > 0 ? (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-start">
                                {filteredStocks.map((stock) => (
                                    <StockCard key={stock.tradingsymbol} stock={stock} />
                                ))}
                            </div>
                        ) : (
                    ...
                    <div className="animate-in fade-in duration-500">
                        <ConnectTab isSyncing={isSyncing} setIsSyncing={setIsSyncing} refreshData={refreshData} token={token} API_URL={API_URL} />
                    </div>
                )}
            </main>
...
const NavItem = ({ icon, label, active, onClick }) => (
    <button onClick={onClick} className={`flex flex-col items-center text-xs font-semibold ${active ? "text-orange-500" : "text-slate-400"}`}>
        {icon}
