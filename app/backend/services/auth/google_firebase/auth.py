...
class Auth:
    ...
    def authenticate(self, email: str, password: str) -> Token:
        ...
        try:
            ...
            raise Exception(
                "Network error: Could not reach the Firebase auth server."
            ) from exe
            ...
            raise Exception(
                f"An unexpected error occurred during auth: {str(e)}"
            ) from e
    ...
    def verify_token(self, token_string: str) -> Token:
        ...
        try:
            ...
            decoded_token = auth.verify_id_token(
                token_string, check_revoked=True)
            return Token(token_string=token_string)
        except auth.InvalidIdTokenError as e:
            # This is where your 'aud' mismatch is caught
            raise Exception(
                f"Invalid Token: Project ID mismatch or expired token. Details: {str(e)}"
            )
        except Exception as e:
