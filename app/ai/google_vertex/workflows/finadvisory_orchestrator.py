...
from app.ai.google_vertex.agents.finadvisory.protfolio.agent import (
    FinAdvisorProtfolioAgent,
)
...
from app.ai.google_vertex.agents.formatter.schemas.protfolio import (
...
class FinAdvisorOrchestrator:
    ...
    def get_pipeline(
            self,
            parent_agent,
            output_schema: any) -> SequentialAgent:
        """
        ...
        return SequentialAgent(
            ...
            sub_agents=[
                FormatterAgent(
                    config=None,
                    output_schema=output_schema).agent(),
            ],
        )
    ...
    async def analyze_stocks(self, user_id: str, stocks_data: dict) -> dict:
        ...
        portfolio_pipeline = self.get_pipeline(
            FinAdvisorStocksAgent().agent(),
            output_schema=StockAnalysisResponse)
        ...
        try:
                ...
                try:
                    async for event in runner.run_async(
                        ...
                        if event.is_final_response():
                            print("[analyze_stocks] Final response received.")
                            break
                    break
            ...
            patch_portfolio_analysis(
                user_id=user_id,
                stock_insights=formatted_data.get("stocks"),
                protfolio_insights=None,
            )
            return formatted_data
        except Exception as e:
            logger.error(f"Portfolio analysis failed: {str(e)}")
            raise PortfolioAnalysisError(
                f"Portfolio analysis failed: {str(e)}") from e

    async def analyze_protfolio(
            self,
            user_id: str,
            protfolio_data: dict) -> dict:
        """
        Executes the portfolio analysis pipeline and stores results.
        ...
        session_service = InMemorySessionService()
        portfolio_pipeline = self.get_pipeline(
            FinAdvisorProtfolioAgent().agent(),
            output_schema=PortfolioAnalysisResponse)
        ...
        try:
            ...
            patch_portfolio_analysis(
                user_id=user_id,
                stock_insights=None,
                protfolio_insights=formatted_data.get("portfolio_insights"),
            )
            return formatted_data
        except Exception as e:
            logger.error(f"Portfolio analysis failed: {str(e)}")
