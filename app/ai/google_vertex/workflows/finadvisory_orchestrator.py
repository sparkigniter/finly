...
from app.ai.google_vertex.agents.tools.firestore_datastore import store_portfolio_analysis
...
from app.ai.errors.error import PortfolioAnalysisError, RateLimitError, DataValidationError
...
class FinAdvisorOrchestrator:
    ...
    async def analyze_portfolio(self, user_id: str, portfolio_data: list):
        ...
        user_message = types.Content(
            role="user", 
            parts=[types.Part.from_text(text=f"Analyze the stocks: {json.dumps(portfolio_data)}")]
        )        
            ...
            try:
                async for event in runner.run_async(
                    ...
                    if event.is_final_response():
                        logger.info(f"Final response event captured for {user_id}")
                ...
                if not formatted_data:
                    raise PortfolioAnalysisError("Pipeline finished but 'formatted_data' missing from state")
                ...
                if "RESOURCE_EXHAUSTED" in err_msg or "429" in err_msg:
                    if attempt < max_retries - 1:
                        ...
                        logger.warning(f"Rate limited (RESOURCE_EXHAUSTED). Retrying in {wait_time}s... (Attempt {attempt+1})")
                        await asyncio.sleep(wait_time)
                        continue
                    else:
                        raise RateLimitError("AI Provider quota exhausted after multiple retries")
                
                # Handle JSON Failures
                if isinstance(e, json.JSONDecodeError):
                    raise DataValidationError(f"Agent returned unparseable JSON: {str(e)}")
                
                # Re-throw unexpected errors for parent logic
                logger.error(f"Critical failure in analyze_portfolio: {str(e)}", exc_info=True)
