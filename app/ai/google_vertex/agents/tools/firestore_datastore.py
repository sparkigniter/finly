...
def patch_portfolio_transaction(transaction, doc_ref, stock_insights: list[dict] | None, protfolio_insights: dict | None):
    ...
    if stock_insights is not None:
        ...
        symbol_to_idx = {s.get("tradingsymbol"): i for i, s in enumerate(existing_stocks)}
        for stock in stock_insights:
            ...
            if symbol in symbol_to_idx:
                ...
                existing_stocks[idx]['technical_view'] = stock.get("technical_view")
                existing_stocks[idx]['fundamental_summary'] = stock.get("fundamental_summary")
                existing_stocks[idx]['recommendation'] = stock.get("recommendation")
            else:
                # INSERT CASE: Append the new stock dictionary if it doesn't exist
...
def store_portfolio_analysis(analysis_data: dict, user_id: str) -> str:
    ...
    try:
        new_id = update_portfolio_transaction(transaction, user_id, analysis_data)
...
def patch_portfolio_analysis(user_id: str, stock_insights: list[dict] | None, protfolio_insights: list[dict] | None):
    """
    ...
    docs = collection_ref.where("user_id", "==", user_id)\
    ...
    try:
        patch_portfolio_transaction(transaction, target_doc_ref, stock_insights, protfolio_insights)
...
def get_stocks(user_id: str) -> dict:
    ...
    results = query.get()
    for doc in results:
        return doc.to_dict().get("analysis_data", {}).get("portfolio", {}).get("stocks", [])
    return []
