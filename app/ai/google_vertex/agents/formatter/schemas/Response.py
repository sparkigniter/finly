...
class IndividualStock(BaseModel):
    ...
    technical_view: str = Field(
        ..., description="Summary of RSI, MACD, and Moving Averages"
    )
    rsi_value: Optional[float] = Field(
        None, description="Relative Strength Index (0-100)"
    )
    fundamental_summary: str = Field(
        ..., description="Summary of P/E, Debt/Equity, and EPS"
    )
    ...
    dividend_yield: float = Field(
        default=0.0, description="Annual dividend yield %")
    market_cap: str = Field(...,
                            description="Market Cap (e.g., Large Cap, Mid Cap)")
